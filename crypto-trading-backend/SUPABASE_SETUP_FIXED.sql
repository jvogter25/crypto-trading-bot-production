-- =====================================================
-- CRYPTO TRADING SYSTEM - SUPABASE DATABASE SETUP (FIXED)
-- =====================================================
-- This version handles existing objects and can be run multiple times safely
...
DROP TRIGGER IF EXISTS update_trading_rules_config_updated_at ON trading_rules_config;
DROP TRIGGER IF EXISTS update_grid_trading_state_updated_at ON grid_trading_state;
...
DROP POLICY IF EXISTS "Allow all operations on sentiment_data" ON sentiment_data;
DROP POLICY IF EXISTS "Allow all operations on sentiment_analysis" ON sentiment_analysis;
DROP POLICY IF EXISTS "Allow all operations on performance_metrics" ON performance_metrics;
DROP POLICY IF EXISTS "Allow all operations on trading_rules_config" ON trading_rules_config;
DROP POLICY IF EXISTS "Allow all operations on grid_trading_state" ON grid_trading_state;
...
-- =====================================================
-- CREATE OR REPLACE PERFORMANCE CALCULATION FUNCTION
-- =====================================================
...
AND table_name IN ('positions', 'market_data', 'sentiment_data', 'sentiment_analysis', 'performance_metrics', 'trading_rules_config', 'grid_trading_state');

-- 2. Check trading rules configuration
SELECT 'Configuration loaded' as status, COUNT(*) as config_count FROM trading_rules_config;

-- 3. Check views exist
SELECT 'Views created' as status, COUNT(*) as view_count 
FROM information_schema.views 
WHERE table_schema = 'public' 
AND table_name IN ('active_positions_with_pnl', 'grid_trading_summary');

-- =====================================================
-- SETUP COMPLETE!
-- =====================================================
SELECT '🎉 Database setup completed successfully!' as message,
       'All tables, indexes, triggers, views, and functions have been created.' as details,
       'Your crypto trading system is now ready for production use.' as status; 